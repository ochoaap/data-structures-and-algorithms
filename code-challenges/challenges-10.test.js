...
const createServer = () => {
  ...
  app.get('/error', status404);

  function aboutmeHandler(request, response) {
    response.send('I am from Wichita, Kansas.');
  }

  function favoritefoodHandler(request, response) {
    response.send(['pizza', 'burgers', 'fries']);
  }
...
const count = (target, input) => {
  return input.reduce((challengeTwo, valueOne) => {
...
const divisibleByFiveTwoToThePower = (input) => {
  return input.map((array) => {
    return array.filter(element => element % 5 === 0 && typeof
    element === 'number').map(result => Math.pow(2, result));
  });
};
